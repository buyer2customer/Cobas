...
class LoginTask {
   ...
   private ArrayList<String> RoleId=new ArrayList<>();
    LoginTask(Context ctx, Dialog load) {
        this.ctx = ctx;
        this.load = load;
    }


    void requestAction(String email, String password) {
        load.dismiss();
        boyprefs = ctx.getSharedPreferences(ctx.getResources().getString(R.string.prefensi), Context.MODE_PRIVATE);
        RequestQueue queue = Volley.newRequestQueue(ctx);

        String POST_ORDER = ctx.getResources().getString(R.string.utama);
                ...
                public void onErrorResponse(VolleyError error) {
                    ...
                    if (error instanceof ServerError && response != null) {
                        try {
                            ...
                            if(hasil.equalsIgnoreCase("Email not yet verified")){
                                ctx.startActivity(new Intent(ctx,Verifikasi.class));
                                ((Activity)ctx).finish();
                            }else{
