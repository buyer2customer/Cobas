...
class RegisterTask {
   ...
   private ArrayList<String> RoleId=new ArrayList<>();
    RegisterTask(Context ctx, Dialog load) {
        this.ctx = ctx;
        this.load = load;
    }


    void requestAction(String title,String fullname,String password,String email,String confirmPassword) {
        load.dismiss();
        boyprefs = ctx.getSharedPreferences(ctx.getResources().getString(R.string.prefensi), Context.MODE_PRIVATE);
        RequestQueue queue = Volley.newRequestQueue(ctx);

        String POST_ORDER = ctx.getResources().getString(R.string.register);
        ...
        try {
            ...
            StringRequest stringRequest = new StringRequest(Request.Method.POST, URL, new Response.Listener<String>() {
                ...
                public String getBodyContentType() {
                    return "application/json; charset=utf-8; accept=*/*";
                }
